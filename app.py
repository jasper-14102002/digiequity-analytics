...
with tab5:
    ...
    def highlight_india(row):
        try:
            if row['country_name'] == 'India':
                return ['background-color: #f3e5f5; font-weight: bold'] * len(row)
            elif pd.notna(row[rank_col]) and int(row[rank_col]) <= 10:
                return ['background-color: #e8f5e9'] * len(row)
            elif pd.notna(row[rank_col]) and int(row[rank_col]) >= len(df_topsis_clean) - 10:
    ...
    st.dataframe(
        df_show.style.apply(highlight_india, axis=1).format(
            {'DIDI Score': '{:.4f}', 'Internet %': '{:.1f}', 'Gini': '{:.1f}'},
            na_rep='N/A'
        ),
        height=380,
